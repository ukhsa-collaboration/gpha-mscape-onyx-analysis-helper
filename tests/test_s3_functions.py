...
def test_make_s3_name(example_result_file):
    s3_key = s3f._make_s3_key_name(analysis_id = "A-1234",
...
def test_upload_file_to_s3(s3_client, test_bucket, example_result_file, example_result_file_sha256):
    ...
    tuple_return = s3f.upload_file_to_s3(analysis_id = "A-1234",
                          bucket = "testbucket",
                          file_for_upload = example_result_file,
                          s3_client = s3_client)
    ...
    response = s3_client.head_object(
        Bucket="testbucket",
        Key="A-1234_C-123456789_qc_results.json"
    )

    assert tuple_return == ("s3://testbucket/A-1234_C-123456789_qc_results.json" , 0)
    assert response['ResponseMetadata']['HTTPHeaders']['x-amz-content-sha256'] == example_result_file_sha256
...
def test_get_s3_checksum(s3_client, test_bucket, example_result_file, example_result_file_sha256):
    ...
    response = s3_client.upload_file(example_result_file, "testbucket", "A-1234_C-123456789_qc_results.json")
    
    tuple_return = s3f.get_s3_checksum("testbucket", "A-1234_C-123456789_qc_results.json", s3_client)
    
    assert tuple_return == (example_result_file_sha256, 0)

@mock_aws
def test_get_s3_checksum_error_handling(s3_client, test_bucket, example_result_file, example_result_file_sha256):
